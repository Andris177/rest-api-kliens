<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold">Új film létrehozása</h2>
    </x-slot>
    ...
    <div class="p-6 max-w-2xl space-y-4">
        ...
        <form method="POST" action="{{ route('movies.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" class="border rounded p-2 w-full" required>
            </div>

            <div>
                <label class="block font-medium mb-1">Leírás</label>
                <textarea name="description" class="border rounded p-2 w-full" rows="5">{{ old('description') }}</textarea>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block font-medium mb-1">Director ID *</label>
                    <input type="number" name="director_id" value="{{ old('director_id') }}" class="border rounded p-2 w-full" required>
                </div>

                <div>
                    <label class="block font-medium mb-1">Category ID *</label>
                    <input type="number" name="category_id" value="{{ old('category_id') }}" class="border rounded p-2 w-full" required>
                </div>
            </div>

            <div>
                <label class="block font-medium mb-1">Cover image (szöveg / útvonal)</label>
                <input type="text" name="cover_image" value="{{ old('cover_image') }}" class="border rounded p-2 w-full" placeholder="pl. films/film_1.jpg">
            </div>
